  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-indigo-100 to-purple-200 flex flex-col items-center justify-center p-4 relative">
      ...
      <div className="w-full max-w-lg flex justify-between items-center mb-3 px-2">
        <div className="flex items-center gap-1.5 text-xs font-bold text-indigo-900 bg-white/70 backdrop-blur-sm px-3 py-1.5 rounded-full border border-indigo-100 shadow-sm">
          <span className="w-2 h-2 rounded-full bg-emerald-500 animate-pulse"></span>
          {user ? (
            <span>👤 მშობელი: {user.user_metadata?.full_name || user.email}</span>
          ) : (
            <span>🎮 სტუმრის რეჟიმი (Guest)</span>
          )}
        </div>
